...
import asvid.counter.analytics.enums.Category.WIDGET
...
class DownCounterWidgetProvider : AppWidgetProvider() {
    ...
    private fun widgetDeleted(context: Context, widgetId: Long) {
        Di.analyticsHelper.sendEvent(WIDGET, DELETE, "")
        val storage = Storage(context)
    ...
    private fun widgetClicked(context: Context, widgetId: Long,
        buttonAction: String) {
        Di.analyticsHelper.sendEvent(WIDGET, Action.CLICKED, "")
        val storage = Storage(context)
        ...
        fun getRemoteViews(context: Context, size: WidgetSize): RemoteViews {
            when (size.widthFactor) {
                1 -> return RemoteViews(context.packageName,
                    layout.counter_widget_layout_1x1)
                2 -> return RemoteViews(context.packageName,
                    layout.counter_widget_layout_1x2)
            }
            if (size.widthFactor!! >= 3) return RemoteViews(context.packageName,
                layout.counter_widget_layout_1x3)
            return RemoteViews(context.packageName, layout.counter_widget_layout_1x1)
        }
